...
import { Trophy, Target, Zap, Star, Award, Gamepad2 } from "lucide-react";
...
export function GamifiedLab() {
  ...
  return (
    <section id="lab" className="relative py-32 overflow-hidden">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[10rem] font-display font-bold text-foreground/[0.02] select-none pointer-events-none">
        LAB
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        >
          <h2 className="font-display font-bold mb-6" style={{fontSize: 'clamp(2.5rem, 5vw, 3.75rem)'}} data-testid="text-lab-title">
            <span className="text-foreground">The </span>
            <span className="bg-gradient-to-r from-neon-purple via-neon-pink to-neon-cyan bg-clip-text text-transparent">
              BlackMonkey Lab
            </span>
          </h2>
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto" data-testid="text-lab-subtitle">
          ...
          >
            {features.map((feature, index) => (
              ...
              >
                ...
                <div>
                  <h3 className="text-xl font-display font-bold text-foreground mb-2" data-testid={`text-lab-feature-title-${index}`}>
                    {feature.title}
                  </h3>
                  <p className="text-muted-foreground leading-relaxed" data-testid={`text-lab-feature-desc-${index}`}>
                    {feature.description}
                  </p>
                </div>
              </motion.div>
            ...
            <Button
              size="lg"
              className="w-full md:w-auto text-lg px-8 py-6 bg-gradient-to-r from-neon-purple to-neon-cyan hover:from-neon-cyan hover:to-neon-purple transition-all duration-300"

...
export function OriginStory() {
  ...
  return (
    <section id="story" ref={containerRef} className="relative py-32 overflow-hidden">
      <motion.div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[6rem] sm:text-[8rem] md:text-[10rem] font-display font-bold text-foreground/[0.02] whitespace-nowrap select-none pointer-events-none"
        style={{ x, opacity }}
      >
        BLACKMONKEY ORIGIN
      </motion.div>
      ...
      <motion.div
        className="absolute top-1/4 left-0 text-[4rem] sm:text-[6rem] font-display font-bold text-foreground/[0.03] select-none pointer-events-none"
        style={{ 
      ...
      <motion.div
        className="absolute bottom-1/4 right-0 text-[4rem] sm:text-[6rem] font-display font-bold text-foreground/[0.03] select-none pointer-events-none"
        style={{ 
          x: useTransform(scrollYProgress, [0, 1], [0, -50]),
          opacity: useTransform(scrollYProgress, [0, 0.3, 0.7, 1], [0, 0.5, 0.5, 0]) 
        }}
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          <p className="font-tech text-xl text-muted-foreground max-w-3xl mx-auto" data-testid="text-story-subtitle">
            From a garage experiment to a STEM revolution. This is how we're changing education forever.
          </p>
        </motion.div>
